...
import select
import datetime
from glider_msg.msg import miniCTmsg

def convert_string_float(dataNMEA):
    return float(dataNMEA[0:4]) + float(dataNMEA[4:]) 
        ...
        try:
            if self.bitstream:
                ...
                if self.dataformat=='NMEA' and self.id != Message.CONFIGURATION_PARAM:
                    ...
                    # skip data units and comma
                    self.dataunits = self.bitstream.read('bytes:2')
                    self.depth = self.bitstream.read('bytes:8')
...
class MiniCT(object):
    """
    *MiniCT* class for sensor miniCT
    """
    def __init__(self, port="/dev/ttyO1", baudrate = 115200):
        """
        :param port:
    ...
    def open(self):
        ...
        if not self.conn:
            try:
                ...
                raise SensorNotFound(self.port,e)

        rospy.loginfo("Initializing miniCT on %s", self.port)
    ...
    def scan(self):
        ...
        while not self.preempted:
            ...
            # msg.range.min_range = self.min_range
            # msg.range.max_range = self.max_range
            try:
                MiniCTmessage.publish(miniCT)
            except:
                pass
...
if __name__ == "__main__":
    rospy.init_node('node_miniCT',log_level=rospy.DEBUG)
    r = rospy.Rate(10)
    port = '/dev/ttyACM0'
    baudrate = 115200
    with MiniCT(port,baudrate) as MiniCT:
        pass
